...
class MovieAPI {
    ...
    async getMovieList(page = 1, preferredSource = 'both') {
        try {
            if (this.useBackend) {
                ...
                if (preferredSource === 'phimapi' || preferredSource === 'both') {
                    fetchPromises.push(this.fetchWithTimeout(`https://phimapi.com/v1/api${endpoint}`, { headers: { 'accept': 'application/json' }, timeout: 8000 }).then(r => r.json()).then(data => ({ data, source: 'phimapi' })));
                }
                if (preferredSource === 'ophim1' || preferredSource === 'both') {
                    fetchPromises.push(this.fetchWithTimeout(`https://ophim1.com/v1/api${endpoint}`, { headers: { 'accept': 'application/json' }, timeout: 8000 }).then(r => r.json()).then(data => ({ data, source: 'ophim1' })));
                }
                    ...
                    if (res.status === 'fulfilled' && res.value) {
                        ...
                        for (const item of items) {
                            if (!seenSlugs.has(item.slug)) {
                                ...
                                if (domainImage) {
                                    if (item.thumb_url && !item.thumb_url.startsWith('http')) {
                                        let path = item.thumb_url.replace(/^\//, '');
                                        if (source === 'ophim1' && !path.startsWith('uploads/')) path = `uploads/movies/${path}`;
                                        item.thumb_url = `${domainImage}/${path}`;
                                    }
                                    if (item.poster_url && !item.poster_url.startsWith('http')) {
                                        let path = item.poster_url.replace(/^\//, '');
                                        if (source === 'ophim1' && !path.startsWith('uploads/')) path = `uploads/movies/${path}`;
                                        item.poster_url = `${domainImage}/${path}`;
                                    }
                                }
    ...
    async searchMovies(keyword, page = 1, preferredSource = 'both') {
        try {
            if (this.useBackend) {
                ...
                let endpoint = `/tim-kiem?keyword=${encodeURIComponent(keyword)}&page=${page}`;
                let fetchPromises = [];
                if (preferredSource === 'phimapi' || preferredSource === 'both') {
                    fetchPromises.push(this.fetchWithTimeout(`https://phimapi.com/v1/api${endpoint}`, { headers: { 'accept': 'application/json' }, timeout: 8000 }).then(r => r.json()).then(data => ({ data, source: 'phimapi' })));
                }
    ...
    async getHome(preferredSource = 'both') {
        try {
            if (this.useBackend) {
