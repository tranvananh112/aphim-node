...
const SEO = {
    ...
    updateMovieSEO(movie, currentEpisode = null) {
        ...
        if (movie.thumb_url) {
            const img = movie.thumb_url.startsWith('http') ? movie.thumb_url : `https://img.ophim.live/ + (.startsWith('uploads/') ?  : 'uploads/movies/' + )`;
    ...
    injectMovieSchema(movie, desc) {
        ...
        const img = movie.thumb_url ? (movie.thumb_url.startsWith('http') ? movie.thumb_url : `https://img.ophim.live/ + (.startsWith('uploads/') ?  : 'uploads/movies/' + )`) : 'https://aphim.io.vn/apple-touch-icon.png';
    ...
    injectItemListSchema(movies) {
        ...
        const itemListElement = movies.slice(0, 30).map((movie, index) => {
            const name = movie.name || movie.title;
            const img = movie.thumb_url ? (movie.thumb_url.startsWith('http') ? movie.thumb_url : `https://img.ophim.live/ + (.startsWith('uploads/') ?  : 'uploads/movies/' + )`) : `${siteOrigin}/apple-touch-icon.png`;
