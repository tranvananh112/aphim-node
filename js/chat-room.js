...
class APFilmChat {
                ...
                window.firebaseChat.onReady(async () => {
                    ...
                    if (this.isBanned) {
                        if (this.el.messageInput) {
                            this.el.messageInput.placeholder = 'Tài kho?n dã b? c?m chat.';
    ...
    _injectHtmlIfNeeded() {
        ...
        if (!document.getElementById('chatContextMenu')) {
            ...
            menu.innerHTML = `
                ...
                <div class="menu-items-container">
                    ...
                    <div class="menu-item danger" id="ctxBan" style="display: none;"><span class="material-icons">block</span> <span>Ch?n ngu?i dùng</span></div>
                    <div class="menu-item danger" id="ctxDelete" style="display: none;"><span class="material-icons">delete</span> <span>Xóa tin nh?n</span></div>
                </div>
            `;
        ...
        const html = `
            <button id="chatFab" class="chat-fab" aria-label="M? chat c?ng d?ng" title="Chat C?ng Ð?ng">
            ...
            <div id="chatWindow" class="chat-window tg-theme" role="dialog">
                <div class="chat-header" id="chatHeader">
                    <div class="chat-header-info">
                        ...
                        <div class="chat-header-text">
                            <div class="chat-header-title">C?ng Ð?ng A Phim</div>
                    ...
                    <div class="chat-header-actions">
                        ...
                        <button class="chat-header-btn" id="chatSearchBtn" title="Tìm ki?m"><span class="material-icons">search</span></button>
                        <button class="chat-header-btn" id="chatMinimizeBtn" title="Thu nh?"><span class="material-icons">remove</span></button>
                        <button class="chat-header-btn" id="chatCloseBtn" title="Ðóng"><span class="material-icons">close</span></button>
                    </div>
                </div>
                ...
                <div class="chat-body" id="chatBody">
                    ...
                    <div class="chat-screen chat-screen-welcome" id="screenWelcome">
                        ...
                        <div>
                            <h2 class="welcome-title">Chào m?ng d?n <span>Kênh A Phim</span></h2>
                            <p class="welcome-desc" style="margin-top:8px;">K?t n?i, th?o lu?n và t?n hu?ng di?n ?nh cùng hàng ngàn tín d? phim ?nh.</p>
                        </div>
                        ...
                        <div class="welcome-features">
                            <div class="welcome-feature"><span class="material-icons">movie</span><div class="welcome-feature-text"><strong>Th?o lu?n phim</strong><span>Review, rating và khám phá phim m?i</span></div></div>
                            <div class="welcome-feature"><span class="material-icons">groups</span><div class="welcome-feature-text"><strong>C?ng d?ng sôi n?i</strong><span>K?t n?i v?i ngu?i xem cùng s? thích</span></div></div>
                            <div class="welcome-feature"><span class="material-icons">bolt</span><div class="welcome-feature-text"><strong>Chat th?i gian th?c</strong><span>Nh?n tin ngay l?p t?c, không d? tr?</span></div></div>
                        </div>
                    ...
                    <div class="chat-screen chat-screen-support" id="screenSupport">
                        <div class="support-inner">
                            ...
                            <p class="support-desc">Chúng tôi luôn s?n sàng h? tr? b?n 24/7.</p>
                ...
                <div class="chat-input-area" id="chatInputArea" style="display: none;">
                    <div class="chat-tabs-bar">
                        ...
                        <button class="chat-tab tab-disabled" data-tab="movies" disabled title="Tính nang dang du?c nâng c?p">
                    ...
                    <div class="input-footer" id="inputFooter" style="display: flex; align-items: center; justify-content: space-between; padding: 6px 2px 0;">
                        ...
                        <a class="change-name-link" id="changeNameLink" style="font-size: 11px; color: var(--c-text-faint); cursor: pointer; text-decoration: underline;">Ð?i tên</a>
                    </div>
                </div>
    ...
    _syncUserUI() {
        ...
        // Hi?n th? tên + badge admin n?u có
        ...
        if (this.el.bulkDeleteBtn) {
            this.el.bulkDeleteBtn.onclick = () => {
                if (confirm(`Xóa ${this.selectedMsgs.length} tin nh?n dã ch?n?`)) {
        ...
        if (this.el.bulkCopyBtn) {
            this.el.bulkCopyBtn.onclick = () => {
                ...
                if (window.showMessage) window.showMessage('Ðã sao chép các tin nh?n du?c ch?n', 'success');
                this._toggleSelectMode(false);
            };
        }
    ...
    _attachEvents() {
        ...
        el.welcomeStartBtn?.addEventListener('click', () => {
            if (this.user) {
                ...
                if (window.authModal && typeof window.authModal.open === 'function') {
                    ...
                    this._showNotification('Vui lòng dang nh?p d? s? d?ng chat', 'warning');
                }
            }
        });
            ...
            tab.addEventListener('click', (e) => {
                ...
                if (tab.disabled || tab.classList.contains('tab-disabled')) {
                    if (window.showMessage) {
                        window.showMessage('?? Tính nang dang du?c nâng c?p', 'info');
                    } else {
                        alert('Tính nang dang du?c nâng c?p');
                    }
                    return;
                }
        ...
        el.bulkForwardBtn?.addEventListener('click', () => {
            if (this.selectedMsgs.length > 0) {
                if (window.showMessage) window.showMessage('Ðã chuy?n ti?p tin nh?n', 'info');
                this._toggleSelectMode(false);
            }
        });
    ...
    _unpinCurrent() {
        if (!confirm('B? ghim tin nh?n này?')) return;
        if (window.firebaseChat?.ready) {
            window.firebaseChat.unpinAll(this.currentTab);
            if (window.showMessage) window.showMessage('Ðã g? ghim tin nh?n', 'info');
        }
    }
    ...
    _jumpToMessage(msgId) {
        ...
        if (target) {
            ...
            if (window.showMessage) window.showMessage('Tin nh?n g?c dã cu ho?c không có trong b? nh?', 'info');
        }
    }
    ...
    _showLoginSuccessNotification() {
        this._showNotification(
            'Ðang nh?p thành công! Vui lòng t?i l?i trang d? s? d?ng chat.',
            'success',
            {
                persistent: true,
                action: {
    ...
    _showContextMenu(e, msg) {
        ...
        document.getElementById('ctxCopy').onclick = () => {
            ...
            menu.style.display = 'none';
            if (window.showMessage) window.showMessage('Ðã sao chép tin nh?n', 'success');
        };
        ...
        if (isAdmin) {
                ...
                if (confirm('Xóa tin nh?n này?')) {
                    try {
                        ...
                        if (token) {
                            ...
                            if (result.success) {
                                if (window.showMessage) {
                                    window.showMessage('? Ðã xóa tin nh?n', 'success');
                                }
                            } else {
                        ...
                        if (window.showMessage) {
                            window.showMessage('? Không th? xóa tin nh?n', 'error');
                        }
                    }
                }
                menu.style.display = 'none';
            ...
            if (msg.isOwn) {
                ctxDelete.style.display = 'flex';
                ctxDelete.onclick = async () => {
                    if (confirm('Xóa tin nh?n c?a b?n?')) {
                        try {
                            // Delete from Firebase first
                            if (window.firebaseChat?.ready) {
                                await window.firebaseChat.deleteMessage(this.currentTab, msg.id);
    ...
    _updateSelectedUI() {
        if (this.el.selectedCount) {
            this.el.selectedCount.textContent = `${this.selectedMsgs.length} dã ch?n`;
        }
        if (this.el.bulkActionBar) {
    ...
    _sendMessage() {
        if (this.isBanned) {
            if (window.showMessage) window.showMessage('B?n dang b? c?m kh?i phòng chat', 'error');
            return;
        }
